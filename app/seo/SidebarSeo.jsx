...
import {
  resetSquenceSeo,
  setPageSequenceSeo,
} from "@app/store/pageSequenceSeo";
...
const SidebarSeo = () => {
  ...
  const [items, setItems] = useState(pageSequenceSeo);
  const page = useSelector((state) => state.pageSeo);
  const dispatch = useDispatch();
  const onDragEnd = (result) => {
    if (!result.destination) return;
    const reorderedItems = Array.from(pageSequenceSeo);
    const [movedItem] = reorderedItems.splice(result.source.index, 1);
    reorderedItems.splice(result.destination.index, 0, movedItem);
    setItems(reorderedItems);
    dispatch(setPageSequenceSeo(reorderedItems));
  };
  const generate = () => {
  ...
  useEffect(() => {
    dispatch(setPageSequenceSeo(items));
  }, [items]);
      ...
      <DragDropContext onDragEnd={onDragEnd}>
        <Droppable droppableId="item_list">
          {(provided) => (
            ...
            >
              {pageSequenceSeo.map((item, index) => (
