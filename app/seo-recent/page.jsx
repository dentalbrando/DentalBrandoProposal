...
import { setPage, updatePage } from "@app/store/pageScliceSeo";
...
function Proposal() {
  ...
  function regenerate(key, inSearch) {
    if (inSearch) {
      ...
      dispatch(
        setBasic(searchData[searchData.length - 1 - key].packages.basic)
      );
      dispatch(
        setStandard(searchData[searchData.length - 1 - key].packages.standard)
      );
      dispatch(
        setPremium(searchData[searchData.length - 1 - key].packages.premium)
      );
      dispatch(
      ...
      dispatch(
        setBasic(proposalData[proposalData.length - 1 - key].packages.basic)
      );
      dispatch(
        setStandard(
          proposalData[proposalData.length - 1 - key].packages.standard
        )
      );
      dispatch(
        setPremium(proposalData[proposalData.length - 1 - key].packages.premium)
      );
  ...
  async function deleteProposal(_id, inSearch) {
    try {
      ...
      let { data } = await axios.post("/api/deleteProposalseo", {
        _id: _id,
      });
